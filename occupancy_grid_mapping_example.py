...
class Map():
    def __init__(self, xsize, ysize, grid_size, occ):
        self.xsize = xsize
        self.ysize = ysize
...
if __name__ == '__main__':
    ...
    df = pd.DataFrame([])
    data = pd.read_excel('//Users//Sami//Library//Containers//com.microsoft.Excel//Data//Desktop//Super'
                  '//Robotics//Project 1 - Jumping right in StudentVersion//Matlab Code//occHard.xlsx')



    # Define the parameters for the map.  (This is a 100x100m map with grid size 1x1m)
    grid_size = 1.0
    map = Map(int(data.shape[0]/grid_size), int(data.shape[1]/grid_size), grid_size,data)
